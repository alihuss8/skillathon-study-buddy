...
def before_request():
    ...
    # Cleanup old sessions (e.g., inactive > 1 hour)
    quiz_state = session['quiz_state']
    now = datetime.now()
    to_remove = []
    for sid, history in answer_histories.items():
        if sid in session and 'last_active' in session['quiz_state']:
            last_active = datetime.fromisoformat(session['quiz_state']['last_active'])
